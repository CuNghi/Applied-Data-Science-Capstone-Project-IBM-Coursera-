...
app.layout = html.Div(children=[
    ...
    dcc.Dropdown(
        id='site-dropdown',  # Add an ID for callbacks
        options=[
        ...
        value='ALL',  # Ensure it matches the options
    ...
    ), # <-- Closing parenthesis for dcc.Dropdo
...
def update_charts(selected_site, payload_range):
    ...
    if selected_site == 'ALL':
        ...
        success_counts['result'] = success_counts['class'].map({1: 'Success', 0: 'Failure'})
        pie_fig = px.pie(
            success_counts,
            values='count',
            names='result',
